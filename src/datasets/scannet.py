...
class ScanNetDataset(utils.data.Dataset):
    ...
    def _read_abs_pose(self, scene_name, name):
        pth = osp.join(os.readlink(osp.join(self.pose_dir, scene_name)),
                       'pose', f'{name}.txt')
    ...
    def __getitem__(self, idx):
        ...
        img_name0 = osp.join(os.readlink(osp.join(self.root_dir, scene_name)), 'color', f'{stem_name_0}.jpg')
        img_name1 = osp.join(os.readlink(osp.join(self.root_dir, scene_name)), 'color', f'{stem_name_1}.jpg')
        print('!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!',img_name0)
        ...
        if self.mode in ['train', 'val']:
            depth0 = read_scannet_depth(osp.join(os.readlink(osp.join(self.root_dir, scene_name)), 'depth', f'{stem_name_0}.png'))
            depth1 = read_scannet_depth(osp.join(os.readlink(osp.join(self.root_dir, scene_name)), 'depth', f'{stem_name_1}.png'))
        else:
